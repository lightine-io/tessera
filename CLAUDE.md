...
- **Orchestrate, don't grind: the main loop is the mastermind, subagents do the work.** The session's main model orchestrates — plans, writes precise task specs, verifies results (diff review, tests, on-device checks) — and delegates implementation to *cheaper-tier* subagents. A well-specified prompt makes the mid tier deliver; reach for the top tier in a subagent only when a task demonstrably needs it and say why. Launching subagents/workflows still requires the maintainer's go-ahead per session norms. Verification stays with the orchestrator — never ship a subagent's work unreviewed. Tiers are *roles, not model names* — resolve them at session start from the model lineup the harness announces: **small** = the cheapest available (mechanical sweeps: greps, scans, candidate-finding), **mid** = the balanced workhorse (implementation batches with a tight spec), **top** = the flagship (only with stated justification). The mastermind role belongs to whichever model holds the main loop — the maintainer's choice, normally the strongest in the plan — and the discipline is identical regardless of who holds it. *(Illustration only, 2026-07: small=Haiku 4.5, mid=Sonnet 5, top=Opus 4.8/Fable 5 — do not treat these names as the rule.)* External (non-Claude) executors are not part of this flow — evaluation tracked in the project's issue tracker; if trialed, isolated worktree + orchestrator-written spec + full diff review before merge.
...
- **Update `CHANGELOG.md` for every non-trivial PR.** Keep a Changelog format. Entries go under `[Unreleased]`, grouped Added / Changed / Deprecated / Removed / Fixed / Security. Trivial PRs (single-character typo fixes etc.) can skip only by carrying the `no-changelog` label — CI checks the label; a description note alone fails the check. Detail in [`.claude/git-workflow.md`](.claude/git-workflow.md).
