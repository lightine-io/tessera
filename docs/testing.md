...
- **Emulator / Simulator — real OCR on a still image** — a synthetic MRZ rendered to an image is fed to the real platform OCR engine to confirm the wiring end-to-end without a camera. **Honest status: only the iOS half of this layer exists.** Apple Vision runs on a supplied image on the Simulator, and that test (`VisionMrzTextRecognizerTest`) executes on CI in the `ios-compile` job. The Android half — ML Kit on a rendered still image, which would need an instrumented test on an emulator — **has not been built**; Android's real-OCR verification has so far happened only at the device layer below. This is a layer the project *intends*, not one it has; treat the Android instrumented-OCR test as an open task (it becomes more pressing with the 0.3.0 still-image work, where the still-image path is the product).
